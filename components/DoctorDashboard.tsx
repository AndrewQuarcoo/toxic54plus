...
export default function DoctorDashboard({ children }: DoctorDashboardProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const pathname = usePathname()

  const navigation = [
    { name: 'Home', href: '/doctor-dashboard', icon: '🏠' },
    { name: 'Patients', href: '/doctor-dashboard/patients', icon: '👥' },
    { name: 'Analysis', href: '/doctor-dashboard/analysis', icon: '🔬' },
    { name: 'Reports', href: '/doctor-dashboard/reports', icon: '📋' },
  ]
        ...
        <div className="flex items-center justify-between h-12 px-4 border-b border-gray-200">
          {!sidebarCollapsed && (
            <div className="text-xl font-bold text-black">
              <span className="italic">MEDICAL</span><span className="text-blue-500">CARE</span>
            </div>
          )}
